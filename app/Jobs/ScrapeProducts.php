...
class ScrapeProducts implements ShouldQueue
{
     ...
     */
    public function __construct($url,$admin_id)
    {
        $this->url=$url;
    ...
    {
        $this->scrape($this->url,$this->admin_id);
    }
    ...
    if(count($matches)>1){
        ...
        Log::info('JSON data not found. Retrying job for product link isset... '.$product_link);
        $this->release(30); // Retry after 30 seconds
        }
    }else{
        Log::info('JSON data not found. Retrying job for product link count... '.$product_link);
        $this->release(30); // Retry after 30 seconds
    }
