...
export default {
  ...
  data(){
    return {
        form:useForm({
            name:this.return_reason.name,
            desc:this.return_reason.desc,
            period:this.return_reason.period,
            status:this.return_reason.status==1?true:false,
        }),
    }
  },
  methods:{
        edit(){
                this.form.put(route('return_reasons.update',this.return_reason.id))
            },
