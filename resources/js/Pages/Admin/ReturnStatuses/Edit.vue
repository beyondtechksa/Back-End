...
export default {
  ...
  data(){
    return {
        form:useForm({
            name:this.return_status.name,
            desc:this.return_status.desc,
            period:this.return_status.period,
            status:this.return_status.status==1?true:false,
        }),
    }
  },
  methods:{
        edit(){
                this.form.put(route('return_statuses.update',this.return_status.id))
            },
