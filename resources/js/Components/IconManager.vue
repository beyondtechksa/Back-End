<template>
    <div class="modal fade" :id="id!=null?id:'iconsModal'"
    data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title" id="staticBackdropLabel">{{__('icons')}}
                </h6>
            ...
            <div class="modal-body">
                <div class="btn-group mb-3 w-100" role="group" aria-label="Basic outlined example">
                    <button @click="show('library')" :class="{'btn-primary text-white':selected_type=='library'}" class="btn btn-outline-primary btn-wave">{{__('library')}}</button>
                    <button @click="show('media')" :class="{'btn-primary text-white':selected_type=='media'}" class="btn btn-outline-primary btn-wave">{{__('media')}}</button>
                    <button  @click="show('custom')" :class="{'btn-primary text-white':selected_type=='custom'}" class="btn btn-outline-primary btn-wave">{{__('custom')}}</button>
                </div>
                <div v-if="selected_type=='library'">
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" placeholder="search icon"
                            aria-label="Username" v-model="search" @keyup="search_icon()" aria-describedby="basic-addon1">
                    </div>
                    </div>
                </div>
                <ul class="icons-list list-unstyled">
                    <li v-for="icon,index in searched_icons" :key="index" @click="select_icon(icon)"
                    class="icons-list-item cursor-pointer">
                        <i :class="icon"></i>
                    </li>
                </ul>
                </div>
                <div v-else-if="selected_type=='media'">
                ...
                <div class="file-manager-navigation">
                    ...
                    <div class="p-3" v-if="newMode || editMode">
                        ...
                        <div class="input-group">
                            <input v-model="form2.name" type="text" class="form-control bg-light border-0" :placeholder="__('folder name')" aria-describedby="button-addon2">
                            <button :disabled="form2.name==''|| form2.busy" class="btn btn-primary" type="submit">
                                <i class="ri-edit-line"></i>
                            </button>
                        </div>
                        <div class="text-danger pt-2 fs-12" v-html="form2.errors.get('name')" />
                    </form>
                    </div>
                ...
                <div class="file-manager-folders">
                    <div class="d-flex p-3 flex-wrap gap-2 align-items-center justify-content-between border-bottom">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <label v-if="!form2.progress" for="fileuploadIcon" class="btn btn-sm btn-outline-secondary  btn-wave waves-light">
                                <i class="ri-upload-line align-middle me-1"></i> {{ __('upload file') }}
                                <input type="file" class="d-none" id="fileuploadIcon" @input="upload_file">
                            </label>
                            <span v-else> {{ __('uploading') }} ...... </span>
                            <div class="text-danger pt-2 fs-12" v-html="form2.errors.get('file')" />

                        </div>
                    </div>
                    <div v-if="form2.progress" class="progress progress-lg mb-5 custom-progress-3 progress-animate" role="progressbar" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100">
                        <div class="progress-bar" :style="'width:'+ form2.progress.percentage +'%'">
                            <div class="progress-bar-value"> {{ form2.progress.percentage }} %</div>
                        </div>
                    </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-xxl-3 col-xl-4 col-lg-6 col-md-6" v-for="file,index in media" :key="index" @dblclick="select_file()">
                                <div class="p-3 text-center uploaded-file" :class="{'selected':selected_media.id==file.id}" @click="selected_media=file">
                                    ...
                                    <div>
                                        <p class="mb-0 fw-semibold fs-16">{{file.name}}</p>
                                        <p class="mb-0 text-muted fs-10">{{file.size}} | 23,Nov 2022</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div v-else-if="selected_type=='custom'">
                    <div class="file-manager-container p-2 gap-2 d-sm-flex">
                <div class="file-manager-navigation">
                    <div class="d-flex align-items-center justify-content-between w-100 p-3 border-bottom">
                        <div>
                            ...
                            <li @click="show_media()" class="files-type" :class="{'active':media_category_id==''}">
                                <a href="javascript:void(0)">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="btn btn-sm btn-icon btn-primary ">{{sum_all()}}</span>
                            ...
                            <li @click="show_media(category)" class="files-type" v-for="category,index in mediaCategories" :key="index" :class="{'active':category.id==media_category_id}">
                                <a href="javascript:void(0)">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <span class="btn btn-sm btn-icon btn-primary ">{{category.media_count}}</span>
                ...
                <div class="file-manager-folders">
                    <div class="d-flex p-3 flex-wrap gap-2 align-items-center justify-content-between border-bottom">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <label v-if="!form2.progress" for="fileupload" class="btn btn-sm btn-outline-secondary  btn-wave waves-light">
                                <i class="ri-upload-line align-middle me-1"></i> {{ __('upload file') }}
...
import Form from "vform"
    ...
    export default{
        ...
        mounted(){
            ...
            this.get_media();
            this.searched_icons=this.icons
        },
        data(){
            return {
                selected_icon:'',
                selected_type:'library',
                newMode:false,
                editMode:false,
                form2:new Form({
                    id:'',
        ...
        methods:{
            ...
            select_icon(icon){
                ...
                this.$emit('iconUploaded')
                let id = this.id!=null?this.id:'iconsModal'
                $('#'+id).modal('hide')
            },
            search_icon(){
                if(this.search){
                    ...
                    this.icons.forEach((e)=>{
                        if(e.search(this.search)!=-1){
                            this.searched_icons.push(e)
                        }
                    })
                }else{
                    this.searched_icons=this.icons
            ...
            editModal(category){
                this.newMode=false
                this.form2.id=category.id
                this.form2.name=category.name
                this.editMode=true
            },
            closeNewModal(){
                this.newMode=false
            ...
            create(){
                ...
                this.form2.post('/admin/api/create_media_category')
                    .then((resp)=>{
                        this.form2.reset()
                        this.newMode=false
            ...
            edit(){
                this.form2.post('/admin/api/update_media_category')
                    .then((resp)=>{
                        this.form2.reset()
                        this.editMode=false
            ...
            del_category(category){
                if(confirm(this.__('are you sure for this action'))){
                    this.form2.id=category.id
                    this.form2.post('/admin/api/delete_media_category')
                    .then((resp)=>{
            ...
            del_media(media){
                if(confirm(this.__('are you sure for this action'))){
                    this.form2.id=media.id
                    this.form2.post('/admin/api/delete_media')
                    .then((resp)=>{
            ...
            get_media_categories(){
                axios.get('/admin/api/get_media_categories/icon')
                     .then((resp)=>{
                        this.mediaCategories=resp.data
                     })
            },
            get_media(){
                axios.get('/admin/api/get_media/icon/'+this.media_category_id)
                     .then((resp)=>{
            ...
            show_media(category=''){
                if(category){
                    ...
                    this.form2.media_category_id=category.id
                }else{
                    this.media_category_id=''
                }
                this.get_media()
            ...
            reader.onload= ()=>{
                this.form2.file=reader.result
            }
            reader.readAsDataURL(file)

            this.form2.post('/admin/api/upload_file')
                      .then((resp)=>{
            ...
            select_file(){
                this.selected_icon=this.selected_media.path
                this.$emit('iconUploaded')
                let id = this.id!=null?this.id:'iconsModal'
                $('#'+id).modal('hide')
            }
